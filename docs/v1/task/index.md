---
title: Tugas
description: Daftar tugas semester ini — dikelompokkan berdasarkan status (aktif dan selesai) serta mata kuliah.
outline: deep
order: 1
---

# Tugas

Daftar tugas semester ini. Dikelompokkan berdasarkan **status** dan **mata kuliah**.

::: info Tentang Halaman Ini
Folder tugas dibagi dua:

- **[Aktif](/v1/task/undone/)** — tugas yang belum/sedang dikerjakan.
- **[Selesai](/v1/task/done/)** — arsip tugas yang sudah dikumpulkan.

Setiap mata kuliah punya halaman indeks tugasnya masing-masing di dalam folder `undone/` atau `done/`.
:::

## Status Tugas

|   Status    | Deskripsi                                  | Jumlah |
| :---------: | ------------------------------------------ | :----: |
|  **Aktif**  | Belum dikumpulkan atau sedang dikerjakan   |   3    |
| **Selesai** | Sudah dikumpulkan (menunggu/sudah dinilai) |   1    |

## Tugas Aktif

| Mata Kuliah                                                                   | Tugas                                                                                                                        | Deadline     | Status |
| ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------ | :----: |
| [RPL103 — Matematika Diskrit](/v1/task/undone/rpl103-matematika-diskrit/)     | [Teori Himpunan](/v1/task/undone/rpl103-matematika-diskrit/13-09-2026-tugas-teori-himpunan)                                  | Sesuai kelas |   ⏳   |
| [RPL106 — Pengantar Basis Data](/v1/task/undone/rpl106-pengantar-basis-data/) | [Tugas Teori Pertemuan 2 — Resume Video ERD](/v1/task/undone/rpl106-pengantar-basis-data/13-09-2026-tugas-teori-pertemuan-2) | Sesuai kelas |   ⏳   |
| [RPL106 — Pengantar Basis Data](/v1/task/undone/rpl106-pengantar-basis-data/) | [Tugas Praktikum Pertemuan 2 — ERD](/v1/task/undone/rpl106-pengantar-basis-data/13-09-2026-tugas-praktikum-pertemuan-2)      | Akhir sesi   |   ⏳   |

## Tugas Selesai

| Mata Kuliah                                                                 | Tugas                                                                                                     | Submitted  |  Grade  |
| --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | :--------: | :-----: |
| [RPL106 — Pengantar Basis Data](/v1/task/done/rpl106-pengantar-basis-data/) | [Resume Video Pertemuan 1](/v1/task/done/rpl106-pengantar-basis-data/13-09-2026-resume-video-pertemuan-1) | 2026-09-13 | Pending |

## Halaman Terkait

| Halaman                                       | Deskripsi                                |
| --------------------------------------------- | ---------------------------------------- |
| [Tugas Aktif](/v1/task/undone/)               | Daftar lengkap tugas yang belum selesai. |
| [Tugas Selesai](/v1/task/done/)               | Arsip tugas yang sudah dikumpulkan.      |
| [Format Halaman Tugas](/format/task)          | Panduan penulisan halaman tugas.         |
| [Format Tugas Selesai](/format/task-complite) | Panduan menandai tugas selesai.          |

---

Task index. Last updated: 2026-09-13.
